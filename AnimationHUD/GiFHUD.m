...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        [self setAlpha:0];
        [self setCenter:[UIApplication sharedApplication].delegate.window.center];
...
+ (void)showWithOverlay {
    [self dismiss:^{
        [[UIApplication sharedApplication].delegate.window addSubview:[[self instance] overlay]];
        [self show];
    }];
}
...
+ (void)show {
    [self dismiss:^{
        [[UIApplication sharedApplication].delegate.window bringSubviewToFront:[self instance]];
...
- (UIView *)overlay {
    ...
    if (!self.overlayView) {
        self.overlayView = [[UIView alloc] initWithFrame:[UIApplication sharedApplication].delegate.window.frame];
